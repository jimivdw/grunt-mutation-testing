...
var IndexHtmlBuilder = function(baseDir, successThreshold){
    ...
    this._successThreshold = successThreshold || DEFAULT_SUCCESS_THRESHOLD;
    this._folderPercentages = {};
};

/**
 * create the index file for the current directory. The index file contains a list of all mutated files in all subdirectories, including links for
 * drilling down into the subdirectories
...
IndexHtmlBuilder.prototype.createIndexFile = function(currentDir, files){
    ...
    _.forEach(files, function(file){
        var relativePath = path.relative(this._baseDir, currentDir),
            ...
            pathSegments = this.linkPathItems({currentDir: relativePath, fileName: file.fileName, separator: '/', filePerc: filePerc});
...
IndexHtmlBuilder.prototype.linkPathItems = function (options) {
    var folderPercentages = this._folderPercentages || {},
        successThreshold = this._successThreshold || DEFAULT_SUCCESS_THRESHOLD,
        relativeLocation = path.relative(options.currentDir, options.fileName),
    ...
    _.forEach(directoryList, function(folder) {
        ...
        linkedPath = linkedPath.concat(segmentLinkTemplate({segment: rawPath, folder: folder, status: folderStatus, separator: options.separator}));
    });

    return linkedPath.concat(fileLinkTemplate({path: relativeLocation, separator: options.separator, file: path.relative(rawPath, relativeLocation), status : fileStatus}));
};
