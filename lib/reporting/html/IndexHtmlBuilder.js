...
var rowTemplate = HandleBars.compile('<tr class="{{status}}"><td>{{{pathSegments}}}</td><td class="perc">{{{filePerc}}}%</td></tr>\n');
...
IndexHtmlBuilder.prototype.createIndexFile = function(currentDir, files) {
    ...
    _.forEach(files, function(file) {
        ...
        rows = rows.concat(rowTemplate({
            ...
            filePerc: filePerc.toFixed(1),
            status: filePerc > this._config.successThreshold ? 'killed' : filePerc > 0 ? 'survived' : 'neutral'
        }));
    }, this);
