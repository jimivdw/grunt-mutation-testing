...
IndexHtmlBuilder.prototype.linkPathItems = function(options) {
    var folderPercentages = this._folderPercentages || {},
        ...
        linkedPath = '',
        fileStatus = options.filePerc !== undefined ? options.filePerc > successThreshold ? 'killed' : 'survived' : '',
        folderStatus;
    ...
    _.forEach(directoryList, function(folder) {
        ...
        folderStatus = perc !== undefined ? perc.total / perc.weight > successThreshold ? 'killed' : 'survived' : '';
    ...
    return linkedPath.concat(fileLinkTemplate({
        ...
        separator: options.separator,
        file: path.relative(rawPath, relativeLocation),
        status: fileStatus
    }));
};
