...
var segmentLinkTemplate = HandleBars.compile('<a class="link {{status}}" href="{{segment}}/index.html">{{folder}}</a>{{{separator}}}');
var fileLinkTemplate = HandleBars.compile('<a class="link {{status}}" href="{{path}}">{{file}}</a>');
...
IndexHtmlBuilder.prototype._getBreadcrumb = function(toDir) {
    ...
    _.forEach(segments, function(folder, i) {
        ...
        breadcrumb = breadcrumb.concat(segmentLinkTemplate({
...
IndexHtmlBuilder.prototype.createIndexFile = function(currentDir, files) {
    var allStats = decorateStatPercentages(_.reduce(files, this._accumulateStatsAndPercentages, {}, this)),
        indexTemplate = HandleBars.compile(fs.readFileSync(__dirname + '/templates/index.hbs', 'utf-8')),
        rowTemplate = HandleBars.compile(fs.readFileSync(__dirname + '/templates/row.hbs', 'utf-8')),
        rows = '',
        breadcrumb = this._getBreadcrumb(currentDir);

    _.forEach(files, function(file) {
        var relativePath = path.relative(this._baseDir, currentDir),
            fileStats = decorateStatPercentages(file.stats),
    ...
    fs.writeFileSync(currentDir + "/index.html", indexTemplate({
        rows: minify(rows),
        stats: allStats,
        style: minify(fs.readFileSync(__dirname + '/templates/index.css', 'utf-8')),
