...
KarmaServerManager.prototype.killAllServerInstances = function() {
    var self = this;
    return killServerInstances(this._serverInstances, function() {
...
function killServerInstances(instances, callback) {
    ...
    _.forEach(instances, function(instance){
        promises.push(Q.Promise(function(resolve){
            spawn('node', [path.join(__dirname, '..', 'lib', 'kill-karma-in-background.js'), instance.process.pid], {stdio: 'pipe'});
            instance.process.on('exit', function(){
                console.log(instance.process.pid, ' exited');
                callback(instance);
                resolve();
            })
        }));
...
/**
 * start a karma server by calling node's "spawn" method.
